...
if [ $(< /proc/sys/net/ipv4/ip_forward) -eq 1 ]
then
	...
	if [[ "${REPLY}" =~ ^[Yy]$ ]] || \
		[ -z "${REPLY}" ]
	then
		sed -iEe '/net\.ipv4\.ip_forward\=1/s/^\#//g' /etc/sysctl.conf
		sysctl -p
...
then
	...
	if iptables -t nat -C POSTROUTING -s "${pivpnNET}/${subnetClass}" -o "${IPv4dev}" -j MASQUERADE -m comment --comment "${VPN}-nat-rule" &> /dev/null
	then
	...
	then
		# Disabled SC Warnings for SC2154, values for variables are sourced from setupVars
		# shellcheck disable=SC2154
		if iptables -C INPUT -i "${IPv4dev}" -p "${pivpnPROTO}" --dport "${pivpnPORT}" -j ACCEPT -m comment --comment "${VPN}-input-rule" &> /dev/null
		then
	...
	then
		# Disabled SC Warnings for SC2154, values for variables are sourced from setupVars
		# shellcheck disable=SC2154
		if iptables -C FORWARD -s "${pivpnNET}/${subnetClass}" -i "${pivpnDEV}" -o "${IPv4dev}" -j ACCEPT -m comment --comment "${VPN}-forward-rule" &> /dev/null
		then
	...
	if LANG='en_US.UTF-8' ufw status | \
		grep -qwsEe 'active'
	then
	...
	if iptables -C ufw-user-input -p "${pivpnPROTO}" --dport "${pivpnPORT}" -j ACCEPT &> /dev/null
	then
	...
	if iptables -C ufw-user-forward -i "${pivpnDEV}" -o "${IPv4dev}" -s "${pivpnNET}/${subnetClass}" -j ACCEPT &> /dev/null
	then
...
then
	...
	if systemctl is-active -q "${VPN_SERVICE}"
	then
		echo ":: [OK] ${VPN_PRETTY_NAME} is running"
	else
		ERR=1
...
then
	if rc-update show default | \
		grep -sEe "\s*${VPN_SERVICE} .*" &> /dev/null
	then
	...
	if systemctl is-enabled -q "${VPN_SERVICE}"
	then
		echo ":: [OK] ${VPN_PRETTY_NAME} is enabled (it will automatically start on reboot)"
	else
		ERR=1
...
if netstat -antu | \
	grep -wqsEe "${pivpnPROTO}.*${pivpnPORT}"
then
