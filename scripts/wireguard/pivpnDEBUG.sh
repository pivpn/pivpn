...
EXAMPLE=$(head -1 /etc/wireguard/configs/clients.txt | \
    awk '{print $1}')
...
then
    ...
    do
        value=$(< "${key}")
        sed -iEe "s/${value}/${key}/" "../configs/${EXAMPLE}.tmp"
    done

    sed -Ee "s/${pivpnHOST}/REDACTED/" < "../configs/${EXAMPLE}.tmp"
