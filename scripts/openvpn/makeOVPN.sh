...
useBitwarden() {
	...
	# check name
	until [[ "${NAME}" =~ ^[a-zA-Z0-9\@\_][a-zA-Z0-9\.\@\_\-]+$ ]]
	do
...
keyPASS() {
	...
	PASSWD=$(echo -n "${PASSWD}" | \
		sed -Ee 's/\\/\\\\/g' -e 's/\//\\\//g' -e 's/\$/\\\$/g' -e 's/!/\\!/g' -e 's/\./\\\./g' -e "s/'/\\\'/g" -e 's/"/\\"/g' -e 's/\*/\\\*/g' -e 's/\@/\\\@/g' -e 's/\#/\\\#/g' -e 's/£/\\£/g' -e 's/%/\\%/g' -e 's/\^/\\\^/g' -e 's/\&/\\\&/g' -e 's/\(/\\\(/g' -e 's/\)/\\\)/g' -e 's/-/\\-/g' -e 's/_/\\_/g' -e 's/\+/\\\+/g' -e 's/=/\\=/g' -e 's/\[/\\\[/g' -e 's/\]/\\\]/g' -e 's/;/\\;/g' -e 's/:/\\:/g' -e 's/\|/\\\|/g' -e 's/\</\\\</g' -e 's/\>/\\\>/g' -e 's/,/\\,/g' -e 's/\?/\\\?/g' -e 's/~/\\~/g' -e 's/\{/\\\{/g' -e 's/\}/\\\}/g')
...
mkdir -p "${install_home}/ovpns"
chown "${install_user}:${install_user}" "${install_home}/ovpns"
chmod 750 "${install_home}/ovpns"
...
then
	...
	if [[ ! "${DAYS}" =~ '^((3650)|(36[0-4][0-9])|(3[0-5][0-9]{2})|[1-2][0-9]{3}|[1-9][0-9]{0,2})$' ]]
	then
...
then
	# Generates the .ovpn file WITHOUT the client private key
	{
		...
		sed -nEe '/\-BEGIN CERTIFICATE\-/,/\-END CERTIFICATE\-/p' < "issued/${NAME}${CRT}"
...
do
	...
	if [ -z $(ls -A /etc/openvpn/ccd) ] || \
		! find /etc/openvpn/ccd -type f -exec grep -q "${NET_REDUCED}.${i}" {} +
	then
		COUNT="${i}"

		extended_subnetmask=$(cidrToMask "${subnetClass}")

		echo "ifconfig-push ${NET_REDUCED}.${i} ${extended_subnetmask}" >> /etc/openvpn/ccd/"${NAME}"
		break
	fi
done

if [ -f /etc/pivpn/hosts.openvpn ]
then
