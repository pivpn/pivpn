...
then
	...
	for ((ii = 0; ii < ${#CERTS_TO_REVOKE[@]}; ii++))
	do
		VALID=0

		for ((x = 1; x <= i; ++x))
		do
			[ "${CERTS[$x]}" == "${CERTS_TO_REVOKE[ii]}" ] && \
				VALID=1
		done
...
do
	...
	then
		...
		rm -rf "pki/reqs/${CERTS_TO_REVOKE[ii]}.req" "pki/private/${CERTS_TO_REVOKE[ii]}.key"
		...
		STATIC_IP=$(grep -vsEe '^\#' "/etc/openvpn/ccd/${CERTS_TO_REVOKE[ii]}" | \
			grep -wsEe 'ifconfig\-push' | \
			grep -osEe "${NET_REDUCED}\.[0-9]{1,3}")

		# disablung warning SC2154, $install_home sourced externally
		# shellcheck disable=SC2154
		rm -rf "/etc/openvpn/ccd/${CERTS_TO_REVOKE[ii]}" "${install_home}/ovpns/${CERTS_TO_REVOKE[ii]}.ovpn"
		...
		then
			sed -iEe "/${STATIC_IP} ${CERTS_TO_REVOKE[ii]}.pivpn/d" /etc/pivpn/hosts.openvpn
